/*
 * Hibernate Search, full-text search for your domain model
 *
 * License: GNU Lesser General Public License (LGPL), version 2.1 or later
 * See the lgpl.txt file in the root directory or <http://www.gnu.org/licenses/lgpl-2.1.html>.
 */
package org.hibernate.search.v6poc.search.dsl.query;

import java.util.function.Consumer;

import org.hibernate.search.v6poc.search.SearchPredicate;
import org.hibernate.search.v6poc.search.dsl.predicate.SearchPredicateContainerContext;

/**
 * @author Yoann Rodiere
 */
public interface SearchQueryResultContext<Q> {

	SearchQueryContext<Q> predicate(SearchPredicate predicate);

	SearchQueryContext<Q> predicate(Consumer<SearchPredicateContainerContext<SearchPredicate>> predicateContributor);

}
