...
and the Elasticsearch backend, delegating to a remove Elasticsearch cluster.
+
...
For example, why expose the the `SearchMappingRepository`?
...
All this is provided to the mapper trough the `MapperFactory` and `Mapper` interfaces.
...
The other part of indexing (or altering the index in any way) is to give an order the index manager:
...
This is necessary to be able to process entities in a straightforard way at runtime,
...
which other entities include that changed entity in their indexed form and should thus be re-indexed.
...
Dependency collectors are organized approximately same way as the indexing processor builders, as a tree.
...
we take care to determine all the possible concrete entity type for the considered type,
